...
export async function generatePDF(valutazione: RisultatoValutazione, datiImmobile: any, leadData: { nome: string; cognome: string; telefono: string; email: string }) {
  // Import dinamico per evitare blocchi al caricamento
  ...
  const formatEuroMq = (val: number) => `€ ${val}/mq`;
  ...
  const docDefinition: any = {
    ...
    content: [
        ...
        table: {
          ...
          body: [
            [
              ...
              { text: 'Valore Massimo', fontSize: 11, bold: true, fillColor: '#E11B22', color: '#FFFFFF', alignment: 'center', margin: [0, 8, 0, 8] },
            ],
            [
      ...
      ...strategia.map((consiglio: string, index: number) => ({
  ...
  (pdfMake as any).createPdf(docDefinition).download('stima-immobiliare-elba.pdf');
}
